...
render_header(
    title="Price Lock",
    subtitle="Автоматический контур возвращает базовую цену WB и скидку продавца к зафиксированным значениям. Продавцовая цена после скидки рассчитывается отдельно.",
...
def prepare_table(dataframe: pd.DataFrame) -> pd.DataFrame:
    columns = {
        ...
        "current_price_retail": "WB база",
        "locked_price_base": "Фикс. WB база",
        "wb_discount": "WB скидка",
        "locked_discount": "Фикс. скидка",
        "current_price": "Текущая продавц.",
        "locked_final_price": "Фикс. продавц.",
        "price_drift": "Откл. продавц.",
        "base_price_drift": "Откл. базы",
        "current_profit": "Прибыль",
        "recommended_price_final": "Рекоменд. продавц.",
        "min_viable_price": "Мин. продавц.",
        "status": "Статус",
    ...
    for col in ["WB скидка", "Фикс. скидка"]:
        if col in table.columns:
            table[col] = pd.to_numeric(table[col], errors="coerce").round(0)
    return table
...
def render_grid(dataframe: pd.DataFrame, key_suffix: str) -> list[dict[str, Any]]:
    ...
    for col in ["WB база", "Фикс. WB база", "Текущая продавц.", "Фикс. продавц.", "Рекоменд. продавц.", "Мин. продавц.", "Прибыль"]:
        if col in table.columns:
            builder.configure_column(col, width=150, type=["numericColumn"], valueFormatter="x == null ? '' : x.toLocaleString('ru-RU') + ' ₽'")
    for col in ["WB скидка", "Фикс. скидка"]:
        if col in table.columns:
            builder.configure_column(col, width=118, type=["numericColumn"], valueFormatter="x == null ? '' : x.toLocaleString('ru-RU') + ' %'")
    for col in ["Откл. продавц.", "Откл. базы"]:
        if col in table.columns:
            builder.configure_column(col, width=138, type=["numericColumn"], valueFormatter="x == null ? '' : x.toLocaleString('ru-RU') + ' ₽'", cellStyle=drift_style)
...
section("Контроль фиксированной цены", "Фиксируется базовая цена WB и скидка продавца. Продавцовая цена = WB база × (1 − скидка). Например: 12 000 ₽ и 30% дают 8 400 ₽, отклонение должно считаться от 8 400 ₽.")
