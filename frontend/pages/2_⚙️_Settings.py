...
render_header(
    ...
    subtitle="Здесь задаются себестоимость, минимальная/желаемая прибыль, базовая цена WB и фиксированная скидка. Продавцовая цена считается как WB база × (1 − скидка).",
...
def configure_grid(subset: pd.DataFrame, tab_key: str):
    ...
    labels = {
        ...
        "tax_rate": "Налог",
        "locked_final_price": "Фикс. цена WB",
...
with info_col:
    st.markdown("<div class='side-panel-muted'>Для фиксации цены товар должен быть активен, иметь включенный Price Lock, фиксированную базовую цену WB и фиксированную скидку. Например, 12 000 ₽ и 30% дают продавцовую цену 8 400 ₽. Экономические поля влияют только на аналитику и рекомендации.</div>", unsafe_allow_html=True)
